...
@endsection
@section('title','Gestionar Archivos')

@section('content')
<div class="block mx-auto my-6 p-8 bg-white  border border-gray-200 rounded-lg shadow-lg">
    <h1 class="text-2xl text-center pt-24">Gestionar archivos PDF</h1>
    <button class="btn btn-primary mb-3" data-toggle="modal" data-target="#exampleModal">Nuevo</button>
    <button class="btn btn-secondary mb-3" data-toggle="modal" data-target="#exampleModal">Buscar</button>
            ...
            @foreach ($theses as $key => $item)
                <tr>
                    ...
                    <td width="25%">
                        ...
                        <button type="button" class="py-2 px-2 rounded-md bg-red-700 hover:bg-red-800" onclick="deleteThesis('{{ $item->id }}')">Eliminar</button>
                    </td>
                    <td>{{ $item->title }}</td>
                    <td>{{ $item->name }}</td>
                    <td>{{ $item->state }}</td>
